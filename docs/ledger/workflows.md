...
```bash
ledger import bank --file ./exports/jan-2025.csv
```
...
- `ledger rules list` prints the current catalogue for review.

## 4. Book the period

```bash
ledger book --period 2025-01
```
...
- Skips periods already locked via `ledger close`.

After booking you can inspect `./.ledger/journal.json` or run `ledger status` to confirm there are no pending transactions.

## 5. Review performance

```bash
ledger report p&l --period 2025-01
ledger vat prepare --period 2025-01
```
...
```bash
ledger export --out ./exports/q1-2025.json --include-audit
```
...
1. `ledger status` – confirm pending transactions and rule coverage.
2. `ledger import bank --file ...` – ingest new statements.
3. `ledger rules add ...` – create rules when new vendors appear.
4. `ledger book --period ...` – book the relevant period (use `--dry-run` first in busy weeks).
5. `ledger report p&l` / `ledger vat prepare` – generate management insights and compliance exports.
