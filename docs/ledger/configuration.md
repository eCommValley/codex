...
All configuration lives in `./.ledger/ledger.yml`. The file uses straightforward YAML and can be edited with any text editor. After manual changes, rerun `codex status` to confirm the CLI parses the file successfully.
...
- Closed periods are appended automatically by `codex close`. Remove entries only if you need to reopen a period intentionally.
