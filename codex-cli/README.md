...
Codex Ledger requires Node.js 18 or newer. After installing globally you can run `ledger` from any project directory.

## Folder layout

- `bin/ledger.js` – entrypoint that invokes the CLI runtime.
...
| Command | Description |
| --- | --- |
| `ledger init` | Initialise `.ledger/` with chart of accounts, VAT profiles and starter rules. |
| `ledger import bank --file` | Import CSV bank statements into the staging area. |
| `ledger rules add` | Define automated categorisation rules (conditions + postings). |
| `ledger book --period` | Apply rules to staged transactions and create journal entries. |
| `ledger report p&l --period` | Generate profit and loss statements. |
| `ledger vat prepare --period` | Summarise input/output VAT per rate. |
| `ledger close --period` | Lock a fiscal period and append to the audit trail. |
| `ledger export --out` | Export ledger data and optional audit log as JSON. |
| `ledger status` | Show company metadata, counts, pending transactions and recent journal entries. |
